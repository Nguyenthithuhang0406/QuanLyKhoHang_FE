...
import { getExportSlipByType, searchExportSlip, updateStatusExportSlip } from "@/api/exportSlipApi/exportSlip";
...
const ListExportSlip = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      let res;
      if (type === "Provider") {
        res = await searchExportSlip(data.exportSlipCode, data.providerId, "", "", data.status, data.timeStart, data.timeEnd, page, limit, data.type);
      } else {
        if (type === "Agency") {
          res = await searchExportSlip(data.exportSlipCode, "", data.providerId, "", data.status, data.timeStart, data.timeEnd, page, limit, data.type);
        }
  ...
  const handleClickPen = (exportSlipId) => {
    navigate(`/infor-exportSlip/${exportSlipId}`);
  };
      ...
      <div className="container_ListExportSlip">
        <div className="sub_ListExportSlip">
          <div className="sub_1_ListExportSlip">
            <div>
              <span>Mã phiếu</span>
              <input type="text" className="input_ListExportSlip" name="exportSlipCode" value={inforSearch.exportSlipCode} onChange={(e) =>handleChangeFieldSearch(e) }/>
              <span>Nguồn nhận</span>
              <select name="providerId" id="" className="input1_ListExportSlip" value={inforSearch.providerId} onChange={(e) => handleChangeFieldSearch(e)}>
            ...
            <div>
              ...
              <select name="status" id="" className="input2_ListExportSlip" value={inforSearch.status} onChange={(e) => handleChangeFieldSearch(e)}>
                <option value=""></option>
              ...
              <input type="date" className="date_ListExportSlip" name="timeStart" value={inforSearch.timeStart} onChange={(e) => handleChangeFieldSearch(e)}/>
              <span className="date_ListExportSlip2">Đến ngày</span>
              <input type="date" className="date_ListExportSlip3" name="timeEnd" value={inforSearch.timeEnd} onChange={(e) => handleChangeFieldSearch(e)}/>
            </div>
          </div>
        ...
        <div className="sub_3_ListExportSlip" onClick={() => navigate(`/created-exportSlip/${type}`)}>
        ...
        <div className="table_ListExportSlip">
          <table className="table2_ListExportSlip">
            ...
            {
              exportSlips.length > 0 && exportSlips.map((exportSlip, index) => (
                <tr key={exportSlip._id}>
                  <td>{ (page-1) * limit + index + 1}</td>
                  <td>{ exportSlip.exportSlipCode}</td>
                  <td>{(type === "Provider" && exportSlip.providerId?.providerName) || (type === "Agency" && exportSlip.agencyId?.agencyName)}</td>
                  <td>{ formatCurrency(exportSlip.exportPrice)}</td>
                  <td>{ formatDate(exportSlip.createdAt)}</td>
                  <td>
                    <select
                      className={
                        exportSlip.status === "PENDING" ? "button1_ListExportSlip" : (exportSlip.status === "DONE" ? "button2_ListExportSlip" : (exportSlip.status === "REJECTED" ? "button3_ListExportSlip" : (exportSlip.status === "CONFIRMED" ? "button_ListExportSlip" : (exportSlip.status === "RETURNED" ? "button4_ListExportSlip" : ""))))}
                      onChange={(e) => handleUpdateStatus(exportSlip._id, e.target.value)}
                    >
                      <option
                        className={exportSlip.status === "PENDING" ? "button1_ListExportSlip" : (exportSlip.status === "DONE" ? "button2_ListExportSlip" : (exportSlip.status === "REJECTED" ? "button3_ListExportSlip" : (exportSlip.status === "CONFIRMED" ? "button_ListExportSlip" : (exportSlip.status === "RETURNED" ? "button4_ListExportSlip" : "") )))}
                        value={exportSlip.status}>
                        {exportSlip.status === "PENDING" ? "Chờ duyệt" : (exportSlip.status === "DONE" ? "Đã nhập" : (exportSlip.status === "REJECTED" ? "Từ chối" : (exportSlip.status === "CONFIRMED" ? "Đã duyệt" : "")))}
                      </option>
                      <option className="button1_ListExportSlip" value="PENDING">Chờ duyệt</option>
                      <option className="button2_ListExportSlip" value="DONE">Đã xuất</option>
                      <option className="button3_ListExportSlip" value="REJECTED">Từ chối</option>
                      <option className="button_ListExportSlip" value="CONFIRMED">Đã duyệt</option>
                      <option className="button4_ListExportSlip" value="RETURNED">Hoàn hàng</option>
                    </select>
                  </td>
                  <td className="purple">
                    <span className="pen_ListExportSlip" onClick={() => handleClickPen(exportSlip._id)}>
                      <i class="fa-solid fa-pen"></i>
                    </span>
                    <span className="bin_ListExportSlip" onClick={() => handleClickBin(exportSlip._id)}>
